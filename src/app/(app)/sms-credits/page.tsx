...
export default function SmsTopUpPage() {
  ...
  const handlePayWithPaystack = async () => {
    ...
    try {
      ...
      const handler = (window as any).PaystackPop.setup({
        ...
        onSuccess: async (response: { reference: string; transaction: any }) => {
          ...
          if (result.success) {
            ...
            toast({ title: 'Credit Failed', description: result.message, variant: 'destructive' });
          }
          setIsPaying(false);
        },
        onCancel: () => {
      ...
      toast({ title: 'Payment Error', description: error.message || 'Could not initialize payment.', variant: 'destructive' });
      setIsPaying(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary/20 via-background to-background p-4 md:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {/* ✅ FIXED: Header uses text-foreground, not transparent gradient */}
        <div>
        ...
        <Card className="backdrop-blur-xl bg-card/70 border-border shadow-xl overflow-hidden relative">
            ...
            <CardContent>
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  ...
                  <span>
                    {(smsBalance + selectedPackage.units + (selectedPackage.bonus || 0)).toLocaleString()} units
                  </span>
                </div>
              </div>
              ...
              <Button
                ...
                disabled={isPaying}
                className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-semibold h-12 text-base shadow-lg"
              >
                {isPaying ? (
                  <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Processing...</>
                ) : (
                  <><ShoppingCart className="mr-2 h-5 w-5" />Pay GH₵{selectedPackage.price} with Paystack</>
                )}
              </Button>
        ...
        <Card className="border border-gray-200 bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="text-base text-gray-900">How SMS Credits Work</CardTitle>
          </CardHeader>
